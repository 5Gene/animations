package com.heytap.sports.trace.worker.chart

import android.content.Context
import android.graphics.*
import android.util.AttributeSet
import android.view.View
import com.heytap.sports.trace.Speed
...
class SpeedChart @JvmOverloads constructor(
    ...
    var speeds: List<Speed> = mutableListOf()
    ...
    fun setSpeedsData(speeds: List<Speed>) {
        this.speeds = speeds
        if (width > 0) {
            convert()
        }
    ...
    private fun convert() {
        ...
        val eh = height / max!!.speed
        path.reset()
        speeds.forEach {
            if (path.isEmpty) {
                path.moveTo((it.time - starTime) * et, it.speed * eh)
            } else {
                path.lineTo((it.time - starTime) * et, it.speed * eh)
            }
        }
    }
    ...
    fun setProgress(progress: Float) {
        progressGradient = if (progress > 0.995F) {
            LinearGradient(0F, 0F, width.toFloat(), 0F, colors, floatArrayOf(1F, 1F), android.graphics.Shader.TileMode.CLAMP)
        } else {
            LinearGradient(0F, 0F, width.toFloat(), 0F, colors, floatArrayOf(progress, progress + 0.0001F), android.graphics.Shader.TileMode.CLAMP)
        }
        postInvalidate()
    }
